...
function Page() {
  ...
  const { data: owned } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => {
      const { data } = await supabase.from("transactions").select("description,amount,created_at,status")
  ...
  const mut = useMutation({
    mutationFn: () => buy({ data: { checkerId: selected.id } }),
    onSuccess: () => { toast.success("Checker purchased."); setSelected(null);
      qc.invalidateQueries({ queryKey: ["wallet"] }); qc.invalidateQueries({ queryKey: ["owned-checkers"] }); },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {data?.map((c) => (
          <button key={c.id} onClick={() => setSelected(c)} className="text-left rounded-xl p-5 bg-card border border-border hover:border-primary/50 transition">
      ...
      <Dialog open={!!selected} onOpenChange={(o) => !o && setSelected(null)}>
        <DialogContent>
          ...
          <Button className="w-full" disabled={mut.isPending} onClick={() => mut.mutate()}>{mut.isPending ? "Processing..." : "Confirm purchase"}</Button>
        </DialogContent>
      </Dialog>
    </div>
  );
